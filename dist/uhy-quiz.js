...
`;function _e(t,o){if(!t)return"#";let a={__FAFSA_URL__:o.FAFSA,__TEMPLATE_LIAISON__:o.TEMPLATE_LIAISON,__TEMPLATE_PROGRAM__:o.TEMPLATE_PROGRAM,__TEMPLATE_TRIO__:o.TEMPLATE_TRIO,__TEMPLATE_FAA__:o.TEMPLATE_FAA,__FAFSA_GUIDE_URL__:o.FAFSA_GUIDE_URL};return a[t]?a[t]:t}function Q(t,o){let a=t.nodes[o];if(!a||a.type!=="yesno")return 0;let l=t.nodes[a.yes],c=t.nodes[a.no],m=null;return l&&l.type==="yesno"&&(m=a.yes),c&&c.type==="yesno"&&(m?m=a.yes:m=a.no),1+(m?Q(t,m):0)}function ge(t,o){return o?`${t}:${o}`:t}function K({container:t,tree:o,config:a,callbacks:l={},initialPathSegments:c=[]}){let m=a?.links??{},A=typeof l.onStep=="function"?l.onStep:()=>{},T=typeof l.onOutcome=="function"?l.onOutcome:()=>{},N=t.innerHTML.trim()||me;t.innerHTML="";let y=document.createElement("div");y.className="uhyq-root",y.setAttribute("role","application"),y.setAttribute("aria-label","Unaccompanied homeless youth determination quiz");let w=document.createElement("div");w.className="uhyq-progress",w.setAttribute("role","status");let h=document.createElement("div");h.className="uhyq-card",h.setAttribute("aria-live","polite");let g=document.createElement("div");g.className="uhyq-sr-only",g.setAttribute("aria-live","polite"),g.setAttribute("aria-atomic","true");let L=document.createElement("div");L.className="uhyq-controls";let _=document.createElement("button");_.type="button",_.className="uhyq-btn uhyq-btn-secondary",_.textContent="Back",_.setAttribute("aria-label","Go to previous question"),_.disabled=!0;let v=document.createElement("button");v.type="button",v.className="uhyq-btn uhyq-btn-secondary",v.textContent="Restart",v.setAttribute("aria-label","Restart quiz"),L.append(_,v),y.append(w,h,L,g),t.appendChild(y);let p={currentId:o.startId,history:[],pathSegments:[]},E=null,G=V(y,()=>{window.confirm("Restart the quiz? Your answers will be cleared.")&&I()});function M(){E&&(E(),E=null)}function Y(e){if(!e)return"";if(e.includes(`

`))return e.split(`

`).map(n=>Y(n.trim())).join("");if(e.includes(`
\u2022`)||e.trim().startsWith("\u2022")){let n=e.split(`
`),i=n.findIndex(s=>s.trim().startsWith("\u2022")),r="";if(i>0){let s=n.slice(0,i).join(" ").trim();s&&(r+=`<p>${s}</p>`)}let d=n.slice(i).flatMap(s=>s.split("\u2022")).map(s=>s.trim()).filter(Boolean);return r+=`<ul class="uhyq-bullet-list">${d.map(s=>`<li>${s}</li>`).join("")}</ul>`,r}return`<p>${e}</p>`}function O(e){return typeof e!="string"||!e.includes(`
`)?e:Y(e.trim())}function oe(e){return typeof e!="string"?e:e.replace(/\[tooltip:([^:]+):([^\]]+)\]/g,(n,i,r)=>{let u=`<svg class="uhyq-tooltip-icon" role="button" tabindex="0" aria-label="More information about ${i}" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><path d="M12 17h.01"></path></svg>`;return`${i}<span class="uhyq-tooltip-wrapper">${u}<span class="uhyq-tooltip-text" role="tooltip">${r}</span></span>`})}function ne(e){return typeof e!="string"?e:e.replace(/\[helplink:([^:]+):([^:]+):([^\]]+)\]/g,(n,i,r,u)=>{let d=`<a href="${r}" target="_blank" rel="noopener noreferrer" class="uhyq-helplink" aria-label="${u}" title="${u}"><svg class="uhyq-helplink-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><path d="M12 17h.01"></path></svg></a>`;return`${i}${d}`})}function ie(e){M(),h.innerHTML="";let n=document.createElement("h2");n.id=`${e.id}-prompt`;let i=e.prompt,r="";if(e.prompt.includes(`
...
`)){let f=i;f.includes("[tooltip:")&&(f=oe(f)),f.includes("[helplink:")&&(f=ne(f)),f.includes(`
`)&&(f=f.split(`
`).join("<br>")),n.innerHTML=f}else n.textContent=i;let u=null;r&&(u=document.createElement("div"),u.className="uhyq-body",u.innerHTML=O(r));let d=document.createElement("div");d.className="uhyq-choices",d.setAttribute("role","group"),d.setAttribute("aria-labelledby",n.id);let s={value:"yes",label:"Yes",nextId:e.yes,segmentValue:"yes"},b={value:"no",label:"No",nextId:e.no,segmentValue:"no"},q=document.createElement("button");q.type="button",q.className="uhyq-btn",q.dataset.answer="yes",q.dataset.uhyqFocus="true",q.textContent=s.label,q.addEventListener("click",()=>S(s));let k=document.createElement("button");k.type="button",k.className="uhyq-btn",k.dataset.answer="no",k.textContent=b.label,k.addEventListener("click",()=>S(b)),d.append(q,k),u?h.append(n,u,d):h.append(n,d),E=z(d),H(h),R(g,e.prompt),A({id:e.id,index:p.history.length+1})}function re(e){M(),h.innerHTML="";let n=document.createElement("h2");n.id=`${e.id}-prompt`,n.textContent=e.prompt;let i=document.createElement("div");i.className="uhyq-choices",i.setAttribute("role","group"),i.setAttribute("aria-labelledby",n.id),(e.options||[]).forEach((r,u)=>{let d={value:r.id||r.label,label:r.label,nextId:r.next,segmentValue:r.id||r.label},s=document.createElement("button");s.type="button",s.className="uhyq-btn",u===0&&(s.dataset.uhyqFocus="true"),s.textContent=r.label,s.addEventListener("click",()=>S(d)),i.appendChild(s)}),h.append(n,i),E=z(i),H(h),R(g,e.prompt),A({id:e.id,index:p.history.length+1})}function ae(e){M(),h.innerHTML="";let n=document.createElement("h2");n.textContent=e.title;let i=document.createElement("div");i.className="uhyq-body",i.innerHTML=O(e.body);let r=document.createElement("div");r.className="uhyq-outcome-actions",(e.actions||[]).forEach(u=>{let d=_e(u.href,m),s=document.createElement("a");s.className="uhyq-btn uhyq-btn-secondary uhyq-link-button",s.href=d,s.target="_blank",s.rel="noopener noreferrer",s.textContent=u.label,r.appendChild(s)}),r.children.length&&r.firstElementChild?.setAttribute("data-uhyq-focus","true"),h.append(n,i,r),R(g,e.title),T({id:e.id,title:e.title}),_.disabled=p.history.length===0,H(h)}let x=null;function Ee(e){clearTimeout(x);let n=y.querySelector(".uhyq-alert");n||(n=document.createElement("div"),n.className="uhyq-alert",y.insertBefore(n,L)),n.textContent=e,x=window.setTimeout(()=>{n&&n.remove()},4e3)}function U(e){if(e.type!=="yesno"){w.textContent="You're all set";return}let n=p.history.filter(u=>{let d=o.nodes[u.id];return d&&d.type==="yesno"}).length,i=n+1,r=n+Q(o,e.id);w.textContent=`Question ${i} of ${r}`}function S(e){let n=o.nodes[p.currentId];if(!n||!e||!e.nextId)return;let i=e.segmentValue??e.value??null;p.history.push({id:n.id,answer:i}),p.pathSegments.push(ge(n.id,i)),p.currentId=e.nextId,C()}function se(){if(!p.history.length)return;let e=p.history.pop();p.pathSegments.pop(),p.currentId=e.id,C()}function I(){p.currentId=o.startId,p.history=[],p.pathSegments=[],x&&clearTimeout(x);let e=y.querySelector(".uhyq-alert");e&&e.remove(),C()}function D(e){if(!(!Array.isArray(e)||!e.length)){I();for(let n of e){if(typeof n!="string"||!n.length)break;let i,r;if(n.includes(":")?[i,r]=n.split(":"):(i=n.slice(0,-1),r=n.slice(-1)),!i)break;let u=o.nodes[p.currentId];if(!u||u.id!==i)break;if(u.type==="buttonGroup"){let b=(u.options||[]).find(q=>(q.id||q.label)===r);if(!b||!b.next)break;S({value:b.id||b.label,nextId:b.next,segmentValue:b.id||b.label});continue}if(u.type!=="yesno")break;let d=r==="y"?"yes":r==="n"?"no":r==="yes"||r==="no"?r:null;if(!d)break;let s=u[d];if(!s||(S({value:d,nextId:s,segmentValue:d}),o.nodes[p.currentId]?.type!=="yesno"&&o.nodes[p.currentId]?.type!=="buttonGroup"))break}}}function C(){let e=o.nodes[p.currentId];if(!e){h.innerHTML=N,_.disabled=!0,w.textContent="Sorry, this quiz is unavailable right now.";return}e.type==="yesno"?(U(e),ie(e),_.disabled=p.history.length===0):e.type==="buttonGroup"?(w.textContent="Select your role to begin",re(e),_.disabled=p.history.length===0):(U(e),ae(e))}return _.addEventListener("click",se),v.addEventListener("click",()=>{window.confirm("Restart the quiz? Your answers will be cleared.")&&I()}),C(),c.length&&D(c),{destroy(){M(),G&&(G(),G=null),x&&clearTimeout(x),t.innerHTML=N},restart:I,applyPath:D}}var X=`.uhyq-card h2 p {
  margin: 0 0 0.75rem;
}
...
`,F=new Map;function we(){if(typeof document>"u"||document.getElementById(J))return;let t=document.createElement("style");t.id=J,t.textContent=X,document.head.appendChild(t)}function B(t,o){if(!o)return t;let a=Array.isArray(t)?[...t]:{...t};return Object.keys(o).forEach(l=>{let c=o[l];c&&typeof c=="object"&&!Array.isArray(c)?a[l]=B(a[l]||{},c):a[l]=c}),a}function Ae(t){return B({},t)}function ve(t){if(typeof t!="string"||!t.startsWith("#"))return[];let a=new URLSearchParams(t.slice(1)).get("path");return a?decodeURIComponent(a).split("-").map(c=>c.trim()).filter(Boolean):[]}function xe(t){t&&(t.innerHTML.trim()||(t.innerHTML=qe))}function W(){F.forEach(t=>{t&&typeof t.destroy=="function"&&t.destroy()}),F.clear()}function Z(){F.forEach(t=>{t&&typeof t.restart=="function"&&t.restart()})}function ee(t={}){if(typeof document>"u")return[];let{selector:o="[data-uhy-quiz]",configOverrides:a={},onStep:l,onOutcome:c}=t;we();let m=B(Ae(j),a),A=ve(window.location.hash),T=Array.from(document.querySelectorAll(o));return T.length?T.map((y,w)=>{if(F.has(y)){let g=F.get(y);return g.restart(),g}xe(y);let h=K({container:y,tree:$,config:m,callbacks:{onStep:l,onOutcome:c},initialPathSegments:w===0?A:[]});return F.set(y,h),h}):(console.warn("UHYQuiz: no containers found for selector",o),[])}var te={init:ee,destroyAll:W,restartAll:Z};typeof window<"u"&&(window.UHYQuiz=te);var ke=te;return he(Fe);})();
