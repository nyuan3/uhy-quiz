...
`)&&(f=f.split(`
`).join("<br>")),n.innerHTML=f}else n.textContent=i;let a=null;r&&(a=document.createElement("div"),a.className="uhyq-body",a.innerHTML=U(r,!0));let d=document.createElement("div");d.className="uhyq-choices",d.setAttribute("role","group"),d.setAttribute("aria-labelledby",n.id);let h={value:"yes",label:"Yes",nextId:e.yes,segmentValue:"yes"},c={value:"no",label:"No",nextId:e.no,segmentValue:"no"},q=document.createElement("button");q.type="button",q.className="uhyq-btn",q.dataset.answer="yes",q.dataset.uhyqFocus="true",q.textContent=h.label,q.addEventListener("click",()=>T(h));let k=document.createElement("button");k.type="button",k.className="uhyq-btn",k.dataset.answer="no",k.textContent=c.label,k.addEventListener("click",()=>T(c)),d.append(q,k),a?y.append(n,a,d):y.append(n,d),E=B(d),R(y),z(g,e.prompt),x({id:e.id,index:p.history.length+1})}function re(e){I(),y.innerHTML="";let n=document.createElement("h2");n.id=`${e.id}-prompt`,n.textContent=e.prompt;let i=document.createElement("div");i.className="uhyq-choices",i.setAttribute("role","group"),i.setAttribute("aria-labelledby",n.id),(e.options||[]).forEach((r,a)=>{let d={value:r.id||r.label,label:r.label,nextId:r.next,segmentValue:r.id||r.label},h=document.createElement("button");h.type="button",h.className="uhyq-btn",a===0&&(h.dataset.uhyqFocus="true"),h.textContent=r.label,h.addEventListener("click",()=>T(d)),i.appendChild(h)}),y.append(n,i),E=B(i),R(y),z(g,e.prompt),x({id:e.id,index:p.history.length+1})}function se(e){I(),y.innerHTML="";let n=document.createElement("h2");n.textContent=e.title;let i=document.createElement("div");i.className="uhyq-body",i.innerHTML=U(e.body,!0);let r=document.createElement("div");r.className="uhyq-outcome-actions",(e.actions||[]).forEach(a=>{let d=_e(a.href,_),h=document.createElement("a");h.className="uhyq-btn uhyq-btn-secondary uhyq-link-button",h.href=d,h.target="_blank",h.rel="noopener noreferrer",h.textContent=a.label,r.appendChild(h)}),r.children.length&&r.firstElementChild?.setAttribute("data-uhyq-focus","true"),y.append(n,i,r),z(g,e.title),L({id:e.id,title:e.title}),b.disabled=p.history.length===0,R(y)}let A=null;function Ee(e){clearTimeout(A);let n=m.querySelector(".uhyq-alert");n||(n=document.createElement("div"),n.className="uhyq-alert",m.insertBefore(n,M)),n.textContent=e,A=window.setTimeout(()=>{n&&n.remove()},4e3)}function D(e){if(e.type!=="yesno"){w.textContent="You're all set";return}let n=p.history.filter(a=>{let d=o.nodes[a.id];return d&&d.type==="yesno"}).length,i=n+1,r=n+K(o,e.id);w.textContent=`Question ${i} of ${r}`}function T(e){let n=o.nodes[p.currentId];if(!n||!e||!e.nextId)return;let i=e.segmentValue??e.value??null;p.history.push({id:n.id,answer:i}),p.pathSegments.push(be(n.id,i)),p.currentId=e.nextId,H()}function ae(){if(!p.history.length)return;let e=p.history.pop();p.pathSegments.pop(),p.currentId=e.id,H()}function C(){p.currentId=o.startId,p.history=[],p.pathSegments=[],A&&clearTimeout(A);let e=m.querySelector(".uhyq-alert");e&&e.remove(),H()}function $(e){if(!(!Array.isArray(e)||!e.length)){C();for(let n of e){if(typeof n!="string"||!n.length)break;let i,r;if(n.includes(":")?[i,r]=n.split(":"):(i=n.slice(0,-1),r=n.slice(-1)),!i)break;let a=o.nodes[p.currentId];if(!a||a.id!==i)break;if(a.type==="buttonGroup"){let c=(a.options||[]).find(q=>(q.id||q.label)===r);if(!c||!c.next)break;T({value:c.id||c.label,nextId:c.next,segmentValue:c.id||c.label});continue}if(a.type!=="yesno")break;let d=r==="y"?"yes":r==="n"?"no":r==="yes"||r==="no"?r:null;if(!d)break;let h=a[d];if(!h||(T({value:d,nextId:h,segmentValue:d}),o.nodes[p.currentId]?.type!=="yesno"&&o.nodes[p.currentId]?.type!=="buttonGroup"))break}}}function H(){let e=o.nodes[p.currentId];if(!e){y.innerHTML=N,b.disabled=!0,w.textContent="Sorry, this quiz is unavailable right now.";return}e.type==="yesno"?(D(e),ie(e),b.disabled=p.history.length===0):e.type==="buttonGroup"?(w.textContent="Select your role to begin",re(e),b.disabled=p.history.length===0):(D(e),se(e))}return b.addEventListener("click",ae),v.addEventListener("click",()=>{window.confirm("Restart the quiz? Your answers will be cleared.")&&C()}),H(),u.length&&$(u),{destroy(){I(),G&&(G(),G=null),A&&clearTimeout(A),t.innerHTML=N},restart:C,applyPath:$}}var J=`.uhyq-card h2 p {
  margin: 0 0 0.75rem;
}
...
`,F=new Map;function we(){if(typeof document>"u"||document.getElementById(W))return;let t=document.createElement("style");t.id=W,t.textContent=J,document.head.appendChild(t)}function Y(t,o){if(!o)return t;let s=Array.isArray(t)?[...t]:{...t};return Object.keys(o).forEach(l=>{let u=o[l];u&&typeof u=="object"&&!Array.isArray(u)?s[l]=Y(s[l]||{},u):s[l]=u}),s}function xe(t){return Y({},t)}function ve(t){if(typeof t!="string"||!t.startsWith("#"))return[];let s=new URLSearchParams(t.slice(1)).get("path");return s?decodeURIComponent(s).split("-").map(u=>u.trim()).filter(Boolean):[]}function Ae(t){t&&(t.innerHTML.trim()||(t.innerHTML=qe))}function Z(){F.forEach(t=>{t&&typeof t.destroy=="function"&&t.destroy()}),F.clear()}function ee(){F.forEach(t=>{t&&typeof t.restart=="function"&&t.restart()})}function te(t={}){if(typeof document>"u")return[];let{selector:o="[data-uhy-quiz]",configOverrides:s={},onStep:l,onOutcome:u}=t;we();let _=Y(xe(V),s),x=ve(window.location.hash),L=Array.from(document.querySelectorAll(o));return L.length?L.map((m,w)=>{if(F.has(m)){let g=F.get(m);return g.restart(),g}Ae(m);let y=X({container:m,tree:j,config:_,callbacks:{onStep:l,onOutcome:u},initialPathSegments:w===0?x:[]});return F.set(m,y),y}):(console.warn("UHYQuiz: no containers found for selector",o),[])}var oe={init:te,destroyAll:Z,restartAll:ee};typeof window<"u"&&(window.UHYQuiz=oe);var ke=oe;return he(Fe);})();
