...
`)){let f=r;f.includes("[tooltip:")&&(f=oe(f)),f.includes(`
`)&&(f=f.split(`
`).join("<br>")),n.innerHTML=f}else n.textContent=r;let c=null;i&&(c=document.createElement("div"),c.className="uhyq-body",c.innerHTML=Y(i));let p=document.createElement("div");p.className="uhyq-choices",p.setAttribute("role","group"),p.setAttribute("aria-labelledby",n.id);let s={value:"yes",label:"Yes",nextId:e.yes,segmentValue:"yes"},g={value:"no",label:"No",nextId:e.no,segmentValue:"no"},q=document.createElement("button");q.type="button",q.className="uhyq-btn",q.dataset.answer="yes",q.dataset.uhyqFocus="true",q.textContent=s.label,q.addEventListener("click",()=>S(s));let k=document.createElement("button");k.type="button",k.className="uhyq-btn",k.dataset.answer="no",k.textContent=g.label,k.addEventListener("click",()=>S(g)),p.append(q,k),c?h.append(n,c,p):h.append(n,p),E=P(p),H(h),R(b,e.prompt),A({id:e.id,index:d.history.length+1})}function ie(e){I(),h.innerHTML="";let n=document.createElement("h2");n.id=`${e.id}-prompt`,n.textContent=e.prompt;let r=document.createElement("div");r.className="uhyq-choices",r.setAttribute("role","group"),r.setAttribute("aria-labelledby",n.id),(e.options||[]).forEach((i,c)=>{let p={value:i.id||i.label,label:i.label,nextId:i.next,segmentValue:i.id||i.label},s=document.createElement("button");s.type="button",s.className="uhyq-btn",c===0&&(s.dataset.uhyqFocus="true"),s.textContent=i.label,s.addEventListener("click",()=>S(p)),r.appendChild(s)}),h.append(n,r),E=P(r),H(h),R(b,e.prompt),A({id:e.id,index:d.history.length+1})}function re(e){I(),h.innerHTML="";let n=document.createElement("h2");n.textContent=e.title;let r=document.createElement("div");r.className="uhyq-body",r.innerHTML=Y(e.body);let i=document.createElement("div");i.className="uhyq-outcome-actions",(e.actions||[]).forEach(c=>{let p=me(c.href,m),s=document.createElement("a");s.className="uhyq-btn uhyq-btn-secondary uhyq-link-button",s.href=p,s.target="_blank",s.rel="noopener noreferrer",s.textContent=c.label,i.appendChild(s)}),i.children.length&&i.firstElementChild?.setAttribute("data-uhyq-focus","true"),h.append(n,r,i),R(b,e.title),T({id:e.id,title:e.title}),_.disabled=d.history.length===0,H(h)}let x=null;function Fe(e){clearTimeout(x);let n=y.querySelector(".uhyq-alert");n||(n=document.createElement("div"),n.className="uhyq-alert",y.insertBefore(n,L)),n.textContent=e,x=window.setTimeout(()=>{n&&n.remove()},4e3)}function U(e){if(e.type!=="yesno"){w.textContent="You're all set";return}let n=d.history.filter(c=>{let p=o.nodes[c.id];return p&&p.type==="yesno"}).length,r=n+1,i=n+$(o,e.id);w.textContent=`Question ${r} of ${i}`}function S(e){let n=o.nodes[d.currentId];if(!n||!e||!e.nextId)return;let r=e.segmentValue??e.value??null;d.history.push({id:n.id,answer:r}),d.pathSegments.push(_e(n.id,r)),d.currentId=e.nextId,C()}function ae(){if(!d.history.length)return;let e=d.history.pop();d.pathSegments.pop(),d.currentId=e.id,C()}function M(){d.currentId=o.startId,d.history=[],d.pathSegments=[],x&&clearTimeout(x);let e=y.querySelector(".uhyq-alert");e&&e.remove(),C()}function D(e){if(!(!Array.isArray(e)||!e.length)){M();for(let n of e){if(typeof n!="string"||!n.length)break;let r,i;if(n.includes(":")?[r,i]=n.split(":"):(r=n.slice(0,-1),i=n.slice(-1)),!r)break;let c=o.nodes[d.currentId];if(!c||c.id!==r)break;if(c.type==="buttonGroup"){let g=(c.options||[]).find(q=>(q.id||q.label)===i);if(!g||!g.next)break;S({value:g.id||g.label,nextId:g.next,segmentValue:g.id||g.label});continue}if(c.type!=="yesno")break;let p=i==="y"?"yes":i==="n"?"no":i==="yes"||i==="no"?i:null;if(!p)break;let s=c[p];if(!s||(S({value:p,nextId:s,segmentValue:p}),o.nodes[d.currentId]?.type!=="yesno"&&o.nodes[d.currentId]?.type!=="buttonGroup"))break}}}function C(){let e=o.nodes[d.currentId];if(!e){h.innerHTML=G,_.disabled=!0,w.textContent="Sorry, this quiz is unavailable right now.";return}e.type==="yesno"?(U(e),ne(e),_.disabled=d.history.length===0):e.type==="buttonGroup"?(w.textContent="Select your role to begin",ie(e),_.disabled=d.history.length===0):(U(e),re(e))}return _.addEventListener("click",ae),v.addEventListener("click",()=>{window.confirm("Restart the quiz? Your answers will be cleared.")&&M()}),C(),u.length&&D(u),{destroy(){I(),N&&(N(),N=null),x&&clearTimeout(x),t.innerHTML=G},restart:M,applyPath:D}}var X=`.uhyq-card h2 p {
  margin: 0 0 0.75rem;
}
...
`,F=new Map;function qe(){if(typeof document>"u"||document.getElementById(J))return;let t=document.createElement("style");t.id=J,t.textContent=X,document.head.appendChild(t)}function z(t,o){if(!o)return t;let a=Array.isArray(t)?[...t]:{...t};return Object.keys(o).forEach(l=>{let u=o[l];u&&typeof u=="object"&&!Array.isArray(u)?a[l]=z(a[l]||{},u):a[l]=u}),a}function we(t){return z({},t)}function Ae(t){if(typeof t!="string"||!t.startsWith("#"))return[];let a=new URLSearchParams(t.slice(1)).get("path");return a?decodeURIComponent(a).split("-").map(u=>u.trim()).filter(Boolean):[]}function ve(t){t&&(t.innerHTML.trim()||(t.innerHTML=ge))}function W(){F.forEach(t=>{t&&typeof t.destroy=="function"&&t.destroy()}),F.clear()}function Z(){F.forEach(t=>{t&&typeof t.restart=="function"&&t.restart()})}function ee(t={}){if(typeof document>"u")return[];let{selector:o="[data-uhy-quiz]",configOverrides:a={},onStep:l,onOutcome:u}=t;qe();let m=z(we(Q),a),A=Ae(window.location.hash),T=Array.from(document.querySelectorAll(o));return T.length?T.map((y,w)=>{if(F.has(y)){let b=F.get(y);return b.restart(),b}ve(y);let h=K({container:y,tree:j,config:m,callbacks:{onStep:l,onOutcome:u},initialPathSegments:w===0?A:[]});return F.set(y,h),h}):(console.warn("UHYQuiz: no containers found for selector",o),[])}var te={init:ee,destroyAll:W,restartAll:Z};typeof window<"u"&&(window.UHYQuiz=te);var xe=te;return pe(ke);})();
